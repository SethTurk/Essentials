...
public class Profile {
	...
	public static Player getPlayer(final String user) {
		String uuid = getUUID(user);
		return new Player(MinecraftServer.getServer(), null, new GameProfile(UUID.fromString(uuid), user), null);
	}
	...
	public static String getUsername(final String uuid) {
		...
		try {
			...
			FMLLog.getLogger().warn("Failed to fetch username from UUID", e);
		}
	...
	public static String getUUID(final String username) {
		...
		try {
			...
			FMLLog.getLogger().warn("Failed to fetch UUID  from username", e);
		}
